...
class SougouNBC():
    def __init__(self, save_file_name='./data/sougou/result/sougou_result_clf_rf.csv'):
    ...
    def validation(self):
        ...
        age_temp_models = {i: Pipeline([('vect', TfidfVectorizer()), ('clf', RandomForestClassifier(n_estimators=i)), ])
                           for i in np.arange(5, 100, 5)}
        age_temp_models_list = sorted(age_temp_models.items(), key=lambda elem: elem[0])
        age_train_data, age_train_target = self.get_train_data(self.age_input)
        for age_temp_model in age_temp_models_list:
            age_result = cross_val_score(age_temp_model[1], age_train_data, age_train_target, cv=5)
            self.my_logger.info('use:%s nodes: %d age:%s' % ('RandomForestClassifier', age_temp_model[0], str(age_result)))
        self.my_logger.info('edu validation')
        edu_temp_models = {i: Pipeline([('vect', TfidfVectorizer()), ('clf', RandomForestClassifier(n_estimators=i)), ])
                           for i in np.arange(5, 100, 5)}
        edu_temp_models_list = sorted(edu_temp_models.items(), key=lambda elem: elem[0])
        for edu_temp_model in edu_temp_models_list:
            age_result = cross_val_score(edu_temp_model[1], age_train_data, age_train_target, cv=5)
            self.my_logger.info('use:%s nodes: %d edu:%s' % ('RandomForestClassifier', edu_temp_model[0], str(age_result)))
        '''
