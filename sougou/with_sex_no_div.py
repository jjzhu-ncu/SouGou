...
class SougouNBC():
    def __init__(self, seg=True, save_file_name='./data/sougou/result/sougou_result_nbc_no_seg.csv'):
        ...
        prefix = '' if os.path.exists('./extra_dict/stop_words_ch.txt') else '.'
        self.mid_result_path = prefix + './data/sougou/result/nbc.no.seg.txt'
        self.my_logger = logger_conf()
        self.my_logger.info('init SougouNBC')
        self.train_file_name = prefix + ('./data/2W.TRAIN.pro.seg.jieba' if seg
                                         else './data/2W.TRAIN.pro.no.seg.jieba')
        self.test_file_name = prefix + ('./data/2W.TEST.pro.seg.jieba' if seg
                                        else './data/2W.TEST.pro.jieba')
        self.stop_word_file_name = prefix + './extra_dict/stop_words_ch.txt'
        self.age_model_save_path = prefix + './model/age_ber_no_seg.model'
        self.gender_model_save_path = prefix + './model/gender_ber_no_seg.model'
        self.edu_model_save_path = prefix + './model/edu_ber_no_seg.model'
        ...
        self.gender_input, self.age_input, self.edu_input = self.get_data()
    ...
    def get_data(self):
        ...
        with open(self.train_file_name, mode='r', encoding='utf-8') as train_file:
            for line in train_file:
                ...
                temp_list.clear()
        end_time = datetime.datetime.now()
        self.my_logger.info('model train completed. cost %d seconds' % (end_time-start_time).seconds)

        return gender_input, age_input, edu_input
    ...
    def validation(self):
        ...
        male_age_grid_search = GridSearchCV(male_age_model_pip, param_grid=model_params, n_jobs=jobs, verbose=ver)
        self.print_grid_search_info(male_age_model_pip, model_params)
        male_age_grid_search.fit(age_train_data, age_train_target)
        ...
        male_edu_grid_search = GridSearchCV(male_edu_model_pip, param_grid=model_params, n_jobs=jobs, verbose=ver)
        self.print_grid_search_info(male_edu_model_pip, model_params)
        male_age_grid_search.fit(edu_train_data, edu_train_target)
        ...
        gender_grid_search = GridSearchCV(gender_model_pip, param_grid=model_params, n_jobs=jobs, verbose=ver)
